...
CREATE TABLE IF NOT EXISTS activation_keys (
    ...
    created_at TEXT NOT NULL,  -- Format: "12 Jan 2025"
...
CREATE TABLE IF NOT EXISTS activation_key_events (
    ...
    metadata TEXT, -- JSON pour stocker des infos supplémentaires
    synced INTEGER NOT NULL DEFAULT 0 CHECK(synced IN (0, 1)),
    sync_date TEXT,
...
CREATE TABLE IF NOT EXISTS algorithm_languages (
    ...
    icon_name TEXT, -- Nom de l'icône pour le frontend
    synced INTEGER NOT NULL DEFAULT 0 CHECK(synced IN (0, 1)),
    sync_date TEXT,
...
CREATE TABLE IF NOT EXISTS templates (
    ...
    preview_url TEXT,
    preview_data TEXT, -- Base64 ou URL de l'aperçu
    downloads INTEGER NOT NULL DEFAULT 0,
    uses INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS template_custom_fields (
    ...
    width INTEGER,
    height INTEGER,
    options TEXT, -- JSON pour les champs de type select
    synced INTEGER NOT NULL DEFAULT 0 CHECK(synced IN (0, 1)),
    sync_date TEXT,
...
INSERT OR IGNORE INTO algorithms (name, description, type, version, status, usage_count, rating, documentation_url, created_at, updated_at) VALUES
...
INSERT OR IGNORE INTO algorithm_languages (algorithm_id, language_name, language_version, downloads, icon_name) VALUES
...
INSERT OR IGNORE INTO templates (name, type, category, description, format, downloads, uses, customizable, created_at, updated_at) VALUES
...
-- Badge scolaire moderne (id 1)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
(1, 'élève'), (1, 'qr-code'), (1, 'moderne');

-- Badge professeur premium (id 2)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
(2, 'professeur'), (2, 'premium'), (2, 'accès');

-- Facture standard (id 3)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
(3, 'facture'), (3, 'professionnel'), (3, 'tva');

-- Badge visiteur temporaire (id 4)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
(4, 'visiteur'), (4, 'temporaire'), (4, 'sécurité');

-- Facture détaillée (id 5)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
(5, 'facture'), (5, 'détaillé'), (5, 'échéance');

-- Certificat de scolarité (id 6)
INSERT OR IGNORE INTO template_tags (template_id, tag) VALUES
...
INSERT OR IGNORE INTO api_keys (name, key_text, key_hash, scope, algorithm_id, status, requests_count, created_at, last_used_at) VALUES
('NovaQuantum Crypt - Production', 'nqc_live_xK9mP2qR4tW8zY3n', 'hash_nqc_001', 'read,write', 1, 'active', 45678, '2026-01-01', datetime('now', '-5 minutes')),
('NovaImageCompress - API', 'nic_live_aB7nC3dE5fG8hJ2k', 'hash_nic_001', 'write', 2, 'active', 123456, '2026-01-15', datetime('now', '-2 minutes')),
('NovaEncrypt - Service', 'ne_live_dE4oF5gH6iJ9kL1m', 'hash_ne_001', 'read,write', 3, 'active', 89123, '2026-01-20', datetime('now', '-15 minutes'));

-- ============================================================================
-- Insertion des clés d'activation initiales
-- ============================================================================

INSERT OR IGNORE INTO activation_keys (key_text, school_name, plan, status, created_at, expires_at, uses, max_uses, hw_lock, two_fa, ip_restrict, auto_revoke, fingerprint, last_used, city, sec_score, revocations, key_hash, activation_method) VALUES
...
-- Événements pour clé 1 (SCO-2025-ABCD-1234)
INSERT OR IGNORE INTO activation_key_events (activation_key_id, event_type, event_message, dot_color, event_time) VALUES
...
(1, 'login', '48 nouvelles sessions actives', '#388bfd', datetime('now', '-1 day') || ' · 09:00');

-- Événements pour clé 2 (SCO-2025-EFGH-5678)
INSERT OR IGNORE INTO activation_key_events (activation_key_id, event_type, event_message, dot_color, event_time) VALUES
...
-- Événements pour clé 3 (SCO-2024-IJKL-9012)
INSERT OR IGNORE INTO activation_key_events (activation_key_id, event_type, event_message, dot_color, event_time) VALUES
...
-- Événements pour clé 4 (SCO-2025-MNOP-3456)
INSERT OR IGNORE INTO activation_key_events (activation_key_id, event_type, event_message, dot_color, event_time) VALUES
...
CREATE TABLE IF NOT EXISTS exam_subjects (
    ...
    author_id INTEGER,
    content TEXT, -- Contenu du sujet (texte ou URL)
    file_url TEXT,
    file_size INTEGER,
    file_type TEXT,
...
-- ============================================================================
-- Table des permissions des administrateurs (pour override individuel)
-- ============================================================================
...
INSERT OR IGNORE INTO permissions (code, label, description, category) VALUES
-- Accès
...
-- ============================================================================
-- Table des flux mensuels (pour les graphiques)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS backup_history (
    ...
    FOREIGN KEY (location_id) REFERENCES backup_locations(id) ON DELETE SET NULL
);
